  ...
  runOnRuntime(getExportRuntime(), () => {
    ...
    try {
      try {
        ...
        if (surface == null || cache.__rnskvExportSurfaceWidth !== width || cache.__rnskvExportSurfaceHeight !== height) {
          surface?.dispose();
        ...
        encoder = RNSkiaVideoModule.createVideoEncoder({
          ...
          audioChannelCount: options.audioChannelCount ?? DEFAULT_AUDIO_CHANNEL_COUNT
        }, videoComposition);
        encoder.prepare();
        frameExtractor = RNSkiaVideoModule.createVideoCompositionFramesExtractorSync(videoComposition);
        ...
        const runPooled = RNSkiaVideoModule.runWithAutoreleasePool ?? (fn => fn());
        ...
        for (let i = 0; i < nbFrames; i++) {
          ...
          runPooled(() => {
            const frames = currentExtractor.decodeCompositionFrames(currentTime);
